/*
 * Epsilon Sort With Manual Kernels
 *
 * Intention:
 * This version implements the same geometric-bucketing idea as the Thrust
 * example, but spells out the GPU pipeline manually.
 *
 * High-Level Algorithm:
 * - Reduce the array to find the minimum value.
 * - Build a histogram over logarithmic buckets.
 * - Scan that histogram to compute output offsets.
 * - Scatter each input value into its bucket region.
