 ...
 *
 * Intention:
 * This file demonstrates a counting-sort style pipeline on the GPU for small
 * integer ranges.
 *
 * High-Level Algorithm:
 * - Build a histogram of input values on the GPU.
 * - Perform an exclusive scan of that histogram on the CPU to obtain output
 *   starting offsets.
 * - Launch a second GPU kernel that places each value into its sorted region
